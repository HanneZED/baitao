...
- 左上角圆形头像：使用 `img/icon.jpg`；favicon 使用 `assets/site-icon.svg` 草莓蛋糕图标。
- 作品橱窗：
...
- 背景音乐：左下角简约播放控制器，默认曲目为 `sound/BGM/幽冥水底.mp3`，用户点击后播放，可切歌、可调节音量，默认音量 10%。
- 攻略区：搜索、分类筛选、四张攻略卡。
- 攻略详情：从首页打开时使用页内详情层，不刷新页面，BGM 会继续播放；直接访问 `guides/` 下页面时仍可作为独立静态页浏览。
- 滚动状态：顶部导航和 BGM 控制器会在页面向下滚动后柔和收缩，减少遮挡。
- 联系方式：
  - QQ：727880194
  - 最终幻想14：水汽白桃@沃仙曦染
  - 闲鱼：乐座灯草
...
- 页脚：显示 `power by 温尔文雅（QQ：1129474282）`。
